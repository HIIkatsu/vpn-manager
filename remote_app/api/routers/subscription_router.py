...
async def get_subscription(request: Request, sub_id: str, os: str = "android", exp: int | None = None, sig: str | None = None, session: AsyncSession = Depends(get_read_session)):
    ...
    if not user.is_active:
        ...
        host = getattr(settings, "WEBHOOK_URL_DOMAIN", request.url.hostname)
        cabinet_url = f"https://{host}/cabinet/{user.telegram_id}"
...
async def web_pay(request: Request, uuid: str, amount: float, session: AsyncSession = Depends(get_write_session)):
    if uuid.isdigit() and len(uuid) < 15:
        user = (await session.execute(select(User).where(User.telegram_id == int(uuid)))).scalars().first()
    else:
...
@router.get("/cabinet/{identifier}")
async def web_cabinet(request: Request, identifier: str, session: AsyncSession = Depends(get_write_session)):
    if identifier.isdigit() and len(identifier) < 15:
        user = (await session.execute(select(User).where(User.telegram_id == int(identifier)))).scalars().first()
    else:
        user = (await session.execute(select(User).where(User.vless_uuid == identifier))).scalars().first()

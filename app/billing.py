...
# 1. РУЧКА СОЗДАНИЯ ПЛАТЕЖА (вызывает фронтенд сайта или Mini App)
...
async def create_vpn_payment(req: PaymentRequest):
    ...
    try:
        ...
        payment = Payment.create({
            ...
            "confirmation": {
                "type": "redirect",
                "return_url": "https://t.me/твой_название_боты" # Поменяй на своего бота
            },
            "capture": True,
            "description": f"Пrodlenie VPN для {req.user_uuid}",
...
# 2. РУЧКА ВЕБХУКА (сюда бьет сама ЮKassa)
@router.post("/api/webhook/yookassa")
async def yookassa_webhook(request: Request):
    ...
    if notification.event == "payment.succeeded":
        ...
        # Сигналим генератору пересобрать конфиги, так как база изменилась

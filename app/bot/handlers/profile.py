...
def get_profile_data(user, webhook_domain: str):
    ...
    if user.is_active:
        ...
        profile_text += (
            ...
            f"<code>{sub_url}</code>\n\n"
            f"<i>📥 Скопируй этот ключ, вставь в Nekobox или FoXray в раздел «Подписки» (Sub URL) и нажми «Обновить».</i>"
        )
    else:
        profile_text += "⚠️ <b>Доступ ограничен.</b> Используйте меню ниже, чтобы оплатить и продлить подписку."

    inline_buttons = [
        [InlineKeyboardButton(text="📖 Инструкция по настройке", url="https://telegra.ph/Instrukciya-po-nastrojke-AnKo-VPN-05-20")],

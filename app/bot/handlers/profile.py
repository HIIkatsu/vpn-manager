...
def get_profile_data(user, webhook_domain: str, traffic_str: str = "0.00 B"):
    ...
    if user.is_active:
        profile_text += (
            "<b>Подключение:</b>\n"
            "1) Нажмите кнопку клиента ниже — импорт откроется автоматически.\n"
            "2) Если авто-импорт не сработал, используйте кнопку копирования ссылки."
        )

        encoded_sub_url = sub_url.replace("#", "%23")
        inline_buttons.extend(
            [
                [InlineKeyboardButton(text="📲 Hiddify", url=f"hiddify://import/{encoded_sub_url}")],
                [InlineKeyboardButton(text="📲 v2rayNG", url=f"v2rayng://install-sub?url={encoded_sub_url}")],
                [InlineKeyboardButton(text="📲 Streisand", url=f"streisand://import/{encoded_sub_url}")],
                [InlineKeyboardButton(text="📋 Скопировать ссылку подписки", copy_text={"text": sub_url})],
            ]
        )
    else:
    ...
    inline_buttons.extend(
        [
            [InlineKeyboardButton(text="📖 Полная инструкция", url=f"https://{webhook_domain}/setup")],
...
async def _fetch_user_traffic(user_vless_uuid: str, telegram_id: int, session) -> str:
    try:
        xray = XrayManager()
...
async def profile_handler(message: Message, user_service: UserService, session) -> None:
    ...
    traffic = await _fetch_user_traffic(user.vless_uuid, user.telegram_id, session)
